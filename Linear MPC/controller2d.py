...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            # Linear model predicitive controller
            ...
            u = np.transpose(np.array([v, yaw/st])) # st is time step
            ...
            # cost function
            xx_current = np.transpose(np.array([x, y, yaw, steer_output])) # steer output is radian angle
            ...
            xx_desired = np.transpose(np.array([x_desired, y_desired, yaw_desired, steer_output_desired]))

            dif = xx_desired - xx_current # error term in the cost function J
            ...
            xx_new = np.matrix(scipy.linalg.solve_discrete_are(A, B, Q, R)) # updated value of xx_current
